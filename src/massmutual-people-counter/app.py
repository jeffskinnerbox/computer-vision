...
video_output = cv2.VideoWriter(args["video_file_out"], fourcc, 20.0, (int(width), int(height)))
...
#                            trc.info({"object":{"id":i.getId(),"direction": "up", "time": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())}})    #noqa
                            trc.feature({"total count": {"enter": cnt_up, "exit": cnt_down, "time": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())}})
